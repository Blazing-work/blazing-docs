...
@app.step
async def send_notification(result: dict, services=None):
    """Send notification about processed data"""
    print(f"Data processed: {result}")
    return result

@app.workflow
async def test_workflow(data: dict, services=None):
    """Orchestrate data processing workflow"""
    processed = await process_data(data, services=services)
    return await send_notification(processed, services=services)

async def main():
    await app.publish()
    result = await app.test_workflow({"data": "test-value"})
...
async def app():
    ...
    test_app = Blazing(api_url="http://localhost:8000", api_token="test-token")
...
async def test_process_data(app):
    ...
    assert result['input']['data'] == "test-value"

async def test_error_handling(app):
    """Test error handling"""
    # Run with invalid input
    try:
        result = await app.test_workflow({"invalid": "data"})
        # Should handle based on step logic
    except Exception as e:
        # Error handling occurs here
        assert 'error' in str(e).lower() or e is not None
```
...
- [Learn about sandbox features](/docs/features-overview)
- [Explore advanced examples](/docs/sandbox-examples)
...
- Check the [Flow Sandbox documentation](/docs/introduction)
