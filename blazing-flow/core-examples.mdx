...
> **Executing Workflows**: The examples show three equivalent ways to execute workflows. Choose whichever style you prefer:
> - `await app.workflow_name(args).wait_result()` - One-liner
> - `run = await app.workflow_name(args); result = await run.result()` - Two-liner with RemoteRun handle (useful when you need the run object)
> - `run = await app.run("workflow_name", args); result = await run.result()` - By name (useful for dynamic workflow names)
...
async def main():
    ...
    await app.publish()
    result = await app.hello("World").wait_result()
...
async def main():
    ...
    await app.publish()
    result = await app.add(10, 20).wait_result()
    print(result)  # 30
...
async def main():
    ...
    await app.publish()
    result = await app.filter_positive([1, -2, 3, -4, 5]).wait_result()
    print(result)  # [1, 3, 5]
...
async def main():
    ...
    await app.publish()
    result = await app.prepare_dataset([1, None, 5, 10, None, 15]).wait_result()
...
async def main():
    ...
    await app.publish()
    result = await app.get_statistics([1, 2, 3, 4, 5]).wait_result()
...
async def main():
    ...
    await app.publish()
    result = await app.process_batch([1, 2, 3, 4, 5]).wait_result()
