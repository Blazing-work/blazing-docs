...
> **Note on Async vs Sync**: All examples below use the recommended async `Blazing` class. If you're learning and prefer synchronous code, see the [Synchronous API guide](/docs/blazing-flow/getting-started/flow-guide#synchronous-api) for `SyncBlazing` (for learning/prototyping only - async is recommended for production).
...
- [Blazing Flow Endpoints](/docs/blazing-flow/endpoints/overview) - Expose workflows as REST APIs
- [Blazing Flow Sandbox](/docs/blazing-flow-sandbox/overview) - Run untrusted code safely
