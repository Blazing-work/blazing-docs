...
async def process_order(order_id: str, services=None):
    ...
    validated = await validate_inventory(order, services=services)

    # Step 3: Process payment
    payment = await process_payment(validated, services=services)

    # Step 4: Ship order
    shipment = await ship_order(payment, services=services)
...
Connectors are injected into services by the runtime and expose access to
external systems (databases, object storage, secrets, web APIs). In Blazing
Flow, you access them via `connectors` inside a service or through the `services`
parameter inside steps. Configuration for persistent infrastructure and
connectors lives in Blazing Core YAML.
