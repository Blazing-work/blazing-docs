...
```
1. Client → Queue Route Task
              ↓
2. Worker → Dequeue Task
              ↓
3. Worker → Execute Route
              ↓
4. Route → Call Stations
              ↓
5. Stations → Use Skillsets
              ↓
...
async def process_order(order_id: str):
    ...
    order = await fetch_order_station(order_id)

    # Step 2: Validate inventory
    validated = await validate_inventory_station(order)

    # Step 3: Process payment
    payment = await process_payment_station(validated)

    # Step 4: Ship order
    shipment = await ship_order_station(payment)
...
3. Worker executes route, calling each station in sequence
4. Each station may spawn its own tasks
...
```python
# Each route execution creates a Unit
unit = Unit(
    route_name="process_order",
    arguments={"order_id": "12345"},
    status="pending",
    created_at=datetime.utcnow()
)

# Each station call creates an Operation
operation = Operation(
    unit_id=unit.id,
    station_name="fetch_order_station",
    status="running",
