...
```python
@app.workflow
async def pipeline(user_id: str, services=None):
    user = await fetch_user(user_id, services=services)

    # Real Python - if statements, loops, everything
    if user['premium']:
        result = await premium_processing(user, services=services)
    else:
        result = await standard_processing(user, services=services)

    for item in result['items']:
        await process_item(item, services=services)
...
✅ **Stateful computations** - Long-lived services and connectors for pooled resources
...
# Define the pipeline with @workflow
@app.workflow
...
### 🔄 Automatic Recovery
Pipelines recover from transient failures with retries and durable job tracking.
...
### 🧩 Dynamic Batching
Batch individual calls into a single execution for GPU and throughput efficiency.

### ⏰ Scheduled Workflows
Run workflows on cron or periodic schedules with `Cron` and `Period`.

### 📓 Notebook Runtime (Pyodide)
Use the Blazing Jupyter kernel to run notebook sessions with isolated Python namespaces and checkpointing.
