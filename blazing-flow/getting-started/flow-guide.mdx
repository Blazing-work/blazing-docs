...
<div className="grid grid-cols-1 md:grid-cols-2 gap-4 my-6">
  ...
  <div className="p-4 rounded-lg border border-gray-200 dark:border-dark-800">
    ...
    <div className="text-sm text-muted-foreground">
      Long-lived services and pooled connectors for shared state
    </div>
  </div>
...
@app.step
async def embed(texts: list[str], services=None) -> list[list[float]]:
    # This function receives a batch of inputs
    return await services["EmbeddingService"].encode(texts)
```

**Notes:**
- Apply `@batched` before `@app.step`
- Return a list with length >= batch size

## Scheduled Workflows

Schedule workflows with `Cron` or `Period`:

```python
from blazing import Cron, Period

@app.workflow(schedule=Cron("0 2 * * *"))  # Daily at 2 AM
async def nightly_report(services=None):
    ...

@app.workflow(schedule=Period(minutes=30))  # Every 30 minutes
...
Customize the executor image per step or workflow:
...
Blazing provides a Jupyter kernel that proxies cells to the notebook runtime.

```bash
python -m blazing_kernel install
```

Then select **“Blazing Flow Sandbox”** as the kernel in Jupyter. Notebook sessions
run in an isolated Pyodide runtime and support checkpointing.
