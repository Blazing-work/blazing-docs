...
Build your first Blazing Flow pipeline in just a few minutes.
...
**That's it!** Blazing Flow uses our managed SaaS infrastructure - no local infrastructure setup required.
...
- Add **dynamic batching** with `@batched` for throughput gains.
- Schedule workflows with `Cron` / `Period`.
- Customize executor images with `Image.executor()` when you need system deps.
